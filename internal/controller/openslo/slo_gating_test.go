...
func gatingDatasource(dsType string) *openslov1.Datasource {
...
func gatingSLO(dsType string, magicAlerting bool) *openslov1.SLO {
...
func gatingClient(s *runtime.Scheme, objs ...client.Object) client.WithWatch {
...
func TestSLOGatesOwnedResourcesByBackend(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "an empty type is rejected above rule generation", datasourceType: "", wantParseError: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ds := gatingDatasource(tt.datasourceType)
			slo := gatingSLO(tt.datasourceType, tt.magicAlerting)

			s := gatingTestScheme(t)
			c := gatingClient(s, ds, slo)
			...
			// The fake client does no API-server defaulting, so this row sees the
			// controller with no safety net, pinning the blast radius that made the
			// missing CRD default critical: the parse gate is above PrometheusRule
			// creation, so rule generation dies entirely, not just the Mimir push.
			// Real clusters never reach here; the CRD defaults the type (envtest).
			if tt.wantParseError {
				_, err := r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
				require.NoError(t, err, "pass 1 only adds the finalizer")

				_, err = r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
				require.Error(t, err, "an unparseable backend must fail loudly, not produce a partial SLO")

				prErr := c.Get(ctx, key, &monitoringv1.PrometheusRule{})
				assert.True(t, apierrors.IsNotFound(prErr),
					"nothing below the parse gate runs, so not even a PrometheusRule is created; got err=%v", prErr)

				got := &openslov1.SLO{}
				require.NoError(t, c.Get(ctx, key, got))
				assert.Equal(t, "False", got.Status.Ready,
					"an SLO that generates nothing must not report itself Ready")
				return
			}
...
// TestSLOTransitionDeletesResourcesTheBackendCannotJustify covers repointing a
// live SLO from Mimir to Thanos. Gating creation alone is not enough: a
// MimirRule left behind carries its own frozen copy of the connection details
// and its controller requeues forever, so it keeps pushing rule groups to the
// old Mimir while the SLO reports Ready=True. Owner references do not help --
// they fire on SLO deletion, not on a backend transition. See spec D7.
func TestSLOTransitionDeletesResourcesTheBackendCannotJustify(t *testing.T) {
	config.NewConfig()

	ds := gatingDatasource("mimir")
	slo := gatingSLO("mimir", true)

	s := gatingTestScheme(t)
	c := gatingClient(s, ds, slo)

	rec := record.NewFakeRecorder(100)
	r := &SLOReconciler{Client: c, Scheme: s, Recorder: rec}
	ctx := context.Background()
	key := types.NamespacedName{Name: "test-slo", Namespace: "default"}
	amcKey := types.NamespacedName{Name: "test-slo-alerting", Namespace: "default"}

	for i := 1; i <= 4; i++ {
		_, err := r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
		require.NoError(t, err, "mimir reconcile pass %d", i)
	}

	require.NoError(t, c.Get(ctx, key, &oskov1alpha1.MimirRule{}),
		"precondition: mimir must have produced a MimirRule to orphan")
	require.NoError(t, c.Get(ctx, amcKey, &oskov1alpha1.AlertManagerConfig{}),
		"precondition: magic alerting on mimir must have produced an AlertManagerConfig")

	live := &openslov1.Datasource{}
	require.NoError(t, c.Get(ctx, types.NamespacedName{Name: "test-ds", Namespace: "default"}, live))
	live.Spec.Type = "thanos"
	require.NoError(t, c.Update(ctx, live))

	drainEvents(rec)

	// Two passes: the first performs the deletions, the second proves they are
	// idempotent, which is the common case since most SLOs never had either.
	for i := 1; i <= 2; i++ {
		_, err := r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
		require.NoError(t, err, "thanos reconcile pass %d must treat IsNotFound as success", i)
	}

	mrErr := c.Get(ctx, key, &oskov1alpha1.MimirRule{})
	assert.True(t, apierrors.IsNotFound(mrErr),
		"thanos has no rule-write API, so the orphaned MimirRule must be deleted, not left pushing to the old Mimir; got err=%v", mrErr)

	amcErr := c.Get(ctx, amcKey, &oskov1alpha1.AlertManagerConfig{})
	assert.True(t, apierrors.IsNotFound(amcErr),
		"thanos has no Alertmanager config API, so the orphaned AlertManagerConfig must be deleted; got err=%v", amcErr)

	got := &openslov1.SLO{}
	require.NoError(t, c.Get(ctx, key, got))
	assert.Equal(t, "True", got.Status.Ready,
		"the PrometheusRule still carries the burn-rate rules, so the SLO stays Ready")
}
